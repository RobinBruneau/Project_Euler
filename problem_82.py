...
for i in range(map.shape[0]):
    for j in range(0,1):
        id_left = i*map.shape[1]+j
        id_left_w = map[i,j]
        result = dijsktra(id_left,id_left_w,table_voisins)
        
        for k in range(map.shape[0]):
            id_right = map.shape[1]-1 + map.shape[1]*k 
            min_left = min(min_left,result[id_right])
        print(min_left)